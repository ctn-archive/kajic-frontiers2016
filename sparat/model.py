...
def sanitize(w):
    """Sanitizes words to be valid Semantic Pointer names."""
...
class SpaRat(ctn_benchmark.Benchmark):
    def params(self):
        ...
        self.default("vocab dimensionality", d=512)
        ...
        self.default("association matrix name",
                     assoc_name='example')
        self.default("percentage of removed associations", th=0.0)
        ...
        self.default("associative memory threshold", assoc_th=0.01)
    ...
    def model(self, p):
        ...
        print('Associations before sparsification:', assoc.sum())
        ...
        print('Associations after sparsification {0:.2f}: {1}'.format(
            p.th, assoc.sum()))

        with spa.Module(seed=p.seed) as model:
            print('Creating Semantic Pointers.')
